...
public class Registrator {
    ...
    public static boolean register(String login, String password, String confirmPassword) throws Exception {
    ...
    private static boolean login(String login) throws WrongLoginException {
        ...
        if (login.length() >= 20 & login.contains(" ")) {
            ...
            throw new WrongLoginException("Wrong login, your login contains ' '");
        } else {
    ...
    private static boolean password(String password, String confirmPassword) throws WrongPasswordException {
        ...
        } else if (password.length() >= 20 && password.contains(" ") & !password.replaceAll("[a-zA-Z]", "").matches("[0-9]+")) {
        ...
        } else if (password.length() >= 20 && !password.replaceAll("[a-zA-Z]", "").matches("[0-9]+")) {
            throw new WrongPasswordException("Wrong password, password is too long and not contains number");
        } else if (password.contains(" ") && !password.replaceAll("[a-zA-Z]", "").matches("[0-9]+")) {
        ...
        } else if (!password.replaceAll("[a-zA-Z]", "").matches("[0-9]+")) {
